...
export const BrokerDashboardScreen = React.forwardRef<DashboardRef, BrokerDashboardScreenProps>(
  ...
  const renderIcon = (iconType: string, isActive: boolean) => {
    ...
    switch (iconType) {
      case 'home':
        return (
          <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={color} viewBox="0 0 24 24" strokeWidth={strokeWidth}>
      ...
      <aside className={`w-64 flex-shrink-0 fixed left-0 top-0 bottom-0 z-30 sm:z-20 transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full sm:translate-x-0'}`} style={{ backgroundColor: '#002F23' }}>
        <div className="h-full flex flex-col">
          ...
          <div className="px-6 py-8">
            <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 'var(--font-weight-semibold)', fontSize: 'var(--font-size-h4)', color: '#FFFFFF', lineHeight: 'var(--line-height-heading)' }}>
              CompraTuParcela
            </h2>
          </div>
    ...
    <div className="p-4 md:p-8">
      <div className="flex items-start justify-between gap-3 mb-6 md:mb-8">
        <div className="min-w-0">
          <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(16px, 4vw, 22px)', fontWeight: 700, color: '#0A0A0A', margin: '0 0 4px', lineHeight: 1.2 }}>Bienvenido, Carlos Perez</h1>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#737373' }}>Visión general de tu actividad como broker.</p>
        </div>
...
function PlanContent() {
  ...
  const usageLimits: Record<string, Array<{ label: string; used: number; limit: number }>> = {
    bronce: [
      { label: 'Propiedades en seguimiento', used: 3, limit: 5 },
      { label: 'Publicaciones destacadas',   used: 1, limit: 1 },
    ],
    plata: [
      { label: 'Propiedades en seguimiento', used: 11, limit: 15 },
      { label: 'Publicaciones destacadas',   used: 2,  limit: 3  },
    ],
    oro: [
      { label: 'Propiedades en seguimiento', used: 23, limit: -1 },
      { label: 'Publicaciones destacadas',   used: 4,  limit: 10 },
    ],
  };
          ...
          {plans.map((plan) => {
            ...
            return (
              <div key={plan.id} className="rounded-2xl p-6 flex flex-col" style={{ backgroundColor: '#FFFFFF', border: isActive ? '2px solid #0A0A0A' : '1px solid #E5E5E5', boxShadow: isActive ? '0 4px 6px -1px rgba(0, 0, 0, 0.1)' : '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}>
                ...
                >
                  {isActive ? 'Plan activo' : 'Contratar plan'}
                </button>
              </div>
            );
          })}
            ...
            {invoicesLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="w-12 h-12 rounded-full flex items-center justify-center mb-3" style={{ backgroundColor: '#F5F5F5' }}>
                  <Calendar className="w-6 h-6" style={{ color: '#A3A3A3' }} />
                </div>
                {invoiceRange ? (
                  <>
