...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'usuarios' as NavSection,       label: 'Usuarios',           icon: Users },
      { id: 'publicaciones' as NavSection,  label: 'Publicaciones',      icon: ClipboardList },
  ...
  return (
    <>
      ...
      >
        ...
        {currentSection === 'usuarios'        && <CTPUsuariosView />}
        {currentSection === 'publicaciones'   && <CTPPublicacionesView />}
      </div>
    </>
  );
}
...
type UsuarioTab = 'inmobiliarias' | 'brokers' | 'compradores';
type AccionUsuario = { tipo: 'suspender' | 'editar'; id: number; nombre: string } | null;

const INMOBILIARIAS_DATA = [
  { id: 1, nombre: 'Vista Natura Propiedades', email: 'contacto@vistanatura.cl', plan: 'Premium', publicaciones: 24, estado: 'activo' as const, registro: '12 ene 2025' },
  { id: 2, nombre: 'Inmobiliaria Verde Sur',   email: 'info@verdesur.cl',         plan: 'Básico',   publicaciones: 8,  estado: 'activo' as const, registro: '3 mar 2025' },
  { id: 3, nombre: 'Patagonia Sur',            email: 'hola@patagoniasur.cl',     plan: 'Básico',   publicaciones: 0,  estado: 'pendiente' as const, registro: '10 jul 2025' },
  { id: 4, nombre: 'Andina Terrenos',          email: 'ventas@andina.cl',         plan: 'Pro',      publicaciones: 15, estado: 'activo' as const, registro: '22 abr 2025' },
  { id: 5, nombre: 'Sur Parcelas Ltda.',       email: 'admin@surparcelas.cl',     plan: 'Premium',  publicaciones: 31, estado: 'activo' as const, registro: '18 feb 2025' },
  { id: 6, nombre: 'Campos del Sur',           email: 'info@camposdelsur.cl',     plan: 'Básico',   publicaciones: 3,  estado: 'inactivo' as const, registro: '5 may 2025' },
];

const BROKERS_USR_DATA = [
  { id: 1, nombre: 'Carlos Pérez',    email: 'c.perez@ctp.cl',    leadsActivos: 12, ultimaActividad: 'Hoy, 09:14',    estado: 'activo' as const, registro: '10 feb 2025' },
  { id: 2, nombre: 'Sofía Ramírez',   email: 's.ramirez@ctp.cl',  leadsActivos: 8,  ultimaActividad: 'Ayer, 17:30',   estado: 'activo' as const, registro: '14 mar 2025' },
  { id: 3, nombre: 'Diego Muñoz',     email: 'd.munoz@ctp.cl',    leadsActivos: 5,  ultimaActividad: 'Hace 2 días',   estado: 'activo' as const, registro: '28 ene 2025' },
  { id: 4, nombre: 'Ana Villanueva',  email: 'a.villanueva@ctp.cl', leadsActivos: 0, ultimaActividad: 'Hace 14 días', estado: 'inactivo' as const, registro: '20 abr 2025' },
  { id: 5, nombre: 'Tomás Espinoza', email: 't.espinoza@ctp.cl',  leadsActivos: 3,  ultimaActividad: 'Hoy, 11:02',   estado: 'activo' as const, registro: '2 jun 2025' },
];

const COMPRADORES_DATA = [
  { id: 1, nombre: 'Laura Vásquez',    email: 'l.vasquez@gmail.com',   ultimaVisita: 'Hoy, 08:45',    favoritos: 4, estado: 'activo' as const, registro: '5 jun 2025' },
  { id: 2, nombre: 'Matías Contreras', email: 'm.contreras@gmail.com', ultimaVisita: 'Ayer, 21:10',   favoritos: 2, estado: 'activo' as const, registro: '12 may 2025' },
  { id: 3, nombre: 'Carla Sepúlveda', email: 'c.sepulveda@gmail.com', ultimaVisita: 'Hace 3 días',   favoritos: 7, estado: 'activo' as const, registro: '28 abr 2025' },
  { id: 4, nombre: 'Felipe Aguilera', email: 'f.aguilera@gmail.com',  ultimaVisita: 'Hace 8 días',   favoritos: 1, estado: 'activo' as const, registro: '15 mar 2025' },
  { id: 5, nombre: 'Valentina Ríos',  email: 'v.rios@outlook.com',    ultimaVisita: 'Hace 30 días',  favoritos: 0, estado: 'inactivo' as const, registro: '3 ene 2025' },
  { id: 6, nombre: 'Rodrigo Castillo', email: 'r.castillo@gmail.com', ultimaVisita: 'Hoy, 13:55',   favoritos: 3, estado: 'activo' as const, registro: '20 jun 2025' },
];

function CTPUsuariosView() {
  const [tab, setTab] = useState<UsuarioTab>('inmobiliarias');
  const [search, setSearch] = useState('');
  const [accion, setAccion] = useState<AccionUsuario>(null);
  const [toast, setToast] = useState<string | null>(null);
  const [inmobiliarias, setInmobiliarias] = useState(INMOBILIARIAS_DATA);
  const [brokers, setBrokers] = useState(BROKERS_USR_DATA);
  const [compradores, setCompradores] = useState(COMPRADORES_DATA);
  ...
  function handleSuspender() {
    if (!accion || accion.tipo !== 'suspender') return;
    const id = accion.id;
    const nombre = accion.nombre;
    if (tab === 'inmobiliarias') setInmobiliarias(prev => prev.map(r => r.id === id ? { ...r, estado: r.estado === 'activo' ? 'inactivo' as const : 'activo' as const } : r));
    if (tab === 'brokers') setBrokers(prev => prev.map(r => r.id === id ? { ...r, estado: r.estado === 'activo' ? 'inactivo' as const : 'activo' as const } : r));
    if (tab === 'compradores') setCompradores(prev => prev.map(r => r.id === id ? { ...r, estado: r.estado === 'activo' ? 'inactivo' as const : 'activo' as const } : r));
    setAccion(null);
    showToast(`Cuenta de ${nombre} actualizada`);
  }

  const tabs: { id: UsuarioTab; label: string; count: number }[] = [
    { id: 'inmobiliarias', label: 'Inmobiliarias', count: inmobiliarias.length },
    { id: 'brokers',       label: 'Brokers',       count: brokers.length },
    { id: 'compradores',   label: 'Compradores',   count: compradores.length },
  ];

  const q = search.toLowerCase();

  const filtInmob = inmobiliarias.filter(r => r.nombre.toLowerCase().includes(q) || r.email.toLowerCase().includes(q));
  const filtBrokers = brokers.filter(r => r.nombre.toLowerCase().includes(q) || r.email.toLowerCase().includes(q));
  const filtCompradores = compradores.filter(r => r.nombre.toLowerCase().includes(q) || r.email.toLowerCase().includes(q));

  const estadoStyle = (estado: string) => ({
    activo:    { bg: '#DCFCE7', color: '#16A34A', label: 'Activo' },
    inactivo:  { bg: '#F5F5F5', color: '#737373', label: 'Inactivo' },
    pendiente: { bg: '#FEF3C7', color: '#B7791F', label: 'Pendiente' },
  }[estado] ?? { bg: '#F5F5F5', color: '#737373', label: estado });
    ...
    <div className="p-4 md:p-8">
      ...
      <div className="relative mb-4 max-w-xs">
        ...
        <input
          ...
          style={{ border: '1px solid #E5E5E5', fontFamily: 'var(--font-body)', color: '#0A0A0A', backgroundColor: '#FFFFFF' }}
        />
      </div>
          ...
          <>
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr style={{ backgroundColor: '#FAFAFA', borderBottom: '1px solid #E5E5E5' }}>
                    {['Inmobiliaria', 'Email', 'Plan', 'Publicaciones', 'Estado', 'Registro', 'Acciones'].map(h => (
                      <th key={h} className="px-5 py-3 text-left" style={{ fontSize: '11px', fontWeight: 600, color: '#737373', fontFamily: 'var(--font-body)', letterSpacing: '0.04em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtInmob.length === 0 ? (
                    <tr><td colSpan={7} className="px-5 py-10 text-center" style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#A3A3A3' }}>Sin resultados</td></tr>
                  ) : filtInmob.map((r, idx) => {
                    const s = estadoStyle(r.estado);
                    return (
                      <tr key={r.id} style={{ borderBottom: idx < filtInmob.length - 1 ? '1px solid #F3F4F6' : 'none' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#FAFAFA'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#FFFFFF'; }}>
                        <td className="px-5 py-4"><span style={{ fontSize: '13px', fontWeight: 600, color: '#0A0A0A', fontFamily: 'var(--font-body)' }}>{r.nombre}</span></td>
                        <td className="px-5 py-4"><span style={{ fontSize: '13px', color: '#737373', fontFamily: 'var(--font-body)' }}>{r.email}</span></td>
                        <td className="px-5 py-4"><span className="px-2 py-0.5 rounded-full text-xs font-semibold" style={{ fontFamily: 'var(--font-body)', backgroundColor: r.plan === 'Premium' ? '#F5F3FF' : r.plan === 'Pro' ? '#EFF6FF' : '#F5F5F5', color: r.plan === 'Premium' ? '#7C3AED' : r.plan === 'Pro' ? '#2563EB' : '#737373' }}>{r.plan}</span></td>
                        <td className="px-5 py-4 text-center"><span style={{ fontSize: '14px', fontWeight: 700, color: '#0A0A0A', fontFamily: 'var(--font-heading)' }}>{r.publicaciones}</span></td>
                        ...
                        <td className="px-5 py-4"><span style={{ fontSize: '12px', color: '#A3A3A3', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>{r.registro}</span></td>
                        <td className="px-5 py-4"><AccionesBtns id={r.id} nombre={r.nombre} estado={r.estado} /></td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="md:hidden">
              {filtInmob.map((r, idx) => {
                const s = estadoStyle(r.estado);
                return (
                  <div key={r.id} className="p-4" style={{ borderBottom: idx < filtInmob.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p style={{ fontWeight: 600, fontSize: '14px', color: '#0A0A0A', fontFamily: 'var(--font-body)', margin: 0 }}>{r.nombre}</p>
                        <p style={{ fontSize: '12px', color: '#737373', fontFamily: 'var(--font-body)', margin: 0 }}>{r.email}</p>
                      </div>
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold flex-shrink-0 ml-2" style={{ fontFamily: 'var(--font-body)', backgroundColor: s.bg, color: s.color }}>{s.label}</span>
                    </div>
                    <div className="flex items-center gap-3 mb-3" style={{ marginLeft: '0' }}>
                      <span className="px-2 py-0.5 rounded-full text-xs font-semibold" style={{ fontFamily: 'var(--font-body)', backgroundColor: r.plan === 'Premium' ? '#F5F3FF' : r.plan === 'Pro' ? '#EFF6FF' : '#F5F5F5', color: r.plan === 'Premium' ? '#7C3AED' : r.plan === 'Pro' ? '#2563EB' : '#737373' }}>{r.plan}</span>
                      <span style={{ fontSize: '12px', color: '#737373', fontFamily: 'var(--font-body)' }}>{r.publicaciones} publicaciones</span>
                    </div>
                    <AccionesBtns id={r.id} nombre={r.nombre} estado={r.estado} />
                  </div>
                );
              })}
            </div>
          ...
          <>
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr style={{ backgroundColor: '#FAFAFA', borderBottom: '1px solid #E5E5E5' }}>
                    {['Broker', 'Email', 'Leads activos', 'Última actividad', 'Estado', 'Registro', 'Acciones'].map(h => (
                      <th key={h} className="px-5 py-3 text-left" style={{ fontSize: '11px', fontWeight: 600, color: '#737373', fontFamily: 'var(--font-body)', letterSpacing: '0.04em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtBrokers.length === 0 ? (
                    <tr><td colSpan={7} className="px-5 py-10 text-center" style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#A3A3A3' }}>Sin resultados</td></tr>
                  ) : filtBrokers.map((r, idx) => {
                    const s = estadoStyle(r.estado);
                    return (
                      <tr key={r.id} style={{ borderBottom: idx < filtBrokers.length - 1 ? '1px solid #F3F4F6' : 'none' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#FAFAFA'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#FFFFFF'; }}>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#F0FDFA' }}>
                              <span style={{ fontSize: '11px', fontWeight: 700, color: '#0D9488' }}>{r.nombre.charAt(0)}</span>
                            </div>
                            <span style={{ fontSize: '13px', fontWeight: 600, color: '#0A0A0A', fontFamily: 'var(--font-body)' }}>{r.nombre}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4"><span style={{ fontSize: '13px', color: '#737373', fontFamily: 'var(--font-body)' }}>{r.email}</span></td>
                        <td className="px-5 py-4 text-center"><span style={{ fontSize: '14px', fontWeight: 700, color: '#0A0A0A', fontFamily: 'var(--font-heading)' }}>{r.leadsActivos}</span></td>
                        <td className="px-5 py-4"><span style={{ fontSize: '13px', color: '#737373', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>{r.ultimaActividad}</span></td>
                        <td className="px-5 py-4"><span className="px-2.5 py-0.5 rounded-full text-xs font-semibold" style={{ fontFamily: 'var(--font-body)', backgroundColor: s.bg, color: s.color }}>{s.label}</span></td>
                        <td className="px-5 py-4"><span style={{ fontSize: '12px', color: '#A3A3A3', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>{r.registro}</span></td>
                        <td className="px-5 py-4"><AccionesBtns id={r.id} nombre={r.nombre} estado={r.estado} /></td>
                      </tr>
            ...
            <div className="md:hidden">
              {filtBrokers.map((r, idx) => {
                const s = estadoStyle(r.estado);
                return (
                  <div key={r.id} className="p-4" style={{ borderBottom: idx < filtBrokers.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#F0FDFA' }}>
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#0D9488' }}>{r.nombre.charAt(0)}</span>
                    ...
                    <p style={{ fontSize: '12px', color: '#737373', fontFamily: 'var(--font-body)', margin: '0 0 8px' }}>{r.leadsActivos} leads activos · Activo {r.ultimaActividad.toLowerCase()}</p>
                    <AccionesBtns id={r.id} nombre={r.nombre} estado={r.estado} />
                  </div>
                );
              })}
          ...
          <>
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr style={{ backgroundColor: '#FAFAFA', borderBottom: '1px solid #E5E5E5' }}>
                    {['Comprador', 'Email', 'Última visita', 'Favoritos', 'Estado', 'Registro', 'Acciones'].map(h => (
                      <th key={h} className="px-5 py-3 text-left" style={{ fontSize: '11px', fontWeight: 600, color: '#737373', fontFamily: 'var(--font-body)', letterSpacing: '0.04em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtCompradores.length === 0 ? (
                    <tr><td colSpan={7} className="px-5 py-10 text-center" style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#A3A3A3' }}>Sin resultados</td></tr>
                  ) : filtCompradores.map((r, idx) => {
                    const s = estadoStyle(r.estado);
                    return (
                      <tr key={r.id} style={{ borderBottom: idx < filtCompradores.length - 1 ? '1px solid #F3F4F6' : 'none' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#FAFAFA'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#FFFFFF'; }}>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#EFF6FF' }}>
                              <span style={{ fontSize: '11px', fontWeight: 700, color: '#2563EB' }}>{r.nombre.charAt(0)}</span>
                            </div>
                            <span style={{ fontSize: '13px', fontWeight: 600, color: '#0A0A0A', fontFamily: 'var(--font-body)' }}>{r.nombre}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4"><span style={{ fontSize: '13px', color: '#737373', fontFamily: 'var(--font-body)' }}>{r.email}</span></td>
                        <td className="px-5 py-4"><span style={{ fontSize: '13px', color: '#737373', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>{r.ultimaVisita}</span></td>
                        <td className="px-5 py-4 text-center"><span style={{ fontSize: '14px', fontWeight: 700, color: '#0A0A0A', fontFamily: 'var(--font-heading)' }}>{r.favoritos}</span></td>
                        <td className="px-5 py-4"><span className="px-2.5 py-0.5 rounded-full text-xs font-semibold" style={{ fontFamily: 'var(--font-body)', backgroundColor: s.bg, color: s.color }}>{s.label}</span></td>
                        <td className="px-5 py-4"><span style={{ fontSize: '12px', color: '#A3A3A3', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>{r.registro}</span></td>
                        <td className="px-5 py-4"><AccionesBtns id={r.id} nombre={r.nombre} estado={r.estado} /></td>
                      </tr>
            ...
            <div className="md:hidden">
              {filtCompradores.map((r, idx) => {
                const s = estadoStyle(r.estado);
                return (
                  <div key={r.id} className="p-4" style={{ borderBottom: idx < filtCompradores.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#EFF6FF' }}>
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#2563EB' }}>{r.nombre.charAt(0)}</span>
                    ...
                    <p style={{ fontSize: '12px', color: '#737373', fontFamily: 'var(--font-body)', margin: '0 0 8px' }}>{r.favoritos} favoritos · {r.ultimaVisita}</p>
                    <AccionesBtns id={r.id} nombre={r.nombre} estado={r.estado} />
                  </div>
                );
              })}
      ...
      {/* Modal suspender/editar */}
      {accion && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={() => setAccion(null)}>
          <div className="w-full max-w-sm rounded-2xl" style={{ backgroundColor: '#FFFFFF', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            <div className="p-6">
              {accion.tipo === 'suspender' ? (
                <>
                  ...
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '18px', fontWeight: 600, color: '#0A0A0A', margin: '0 0 8px' }}>
                    {(() => {
                      const currentEstado =
                        tab === 'inmobiliarias' ? inmobiliarias.find(r => r.id === accion.id)?.estado :
                        tab === 'brokers' ? brokers.find(r => r.id === accion.id)?.estado :
                        compradores.find(r => r.id === accion.id)?.estado;
                      return currentEstado === 'activo' ? `¿Suspender a ${accion.nombre}?` : `¿Reactivar a ${accion.nombre}?`;
                    })()}
                  </h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#737373', lineHeight: '1.6', margin: '0 0 20px' }}>
                    {(() => {
                      const currentEstado =
                        tab === 'inmobiliarias' ? inmobiliarias.find(r => r.id === accion.id)?.estado :
                        tab === 'brokers' ? brokers.find(r => r.id === accion.id)?.estado :
                        compradores.find(r => r.id === accion.id)?.estado;
                      return currentEstado === 'activo'
                        ? 'La cuenta quedará suspendida y el usuario no podrá acceder a la plataforma hasta que se reactive.'
                        : 'La cuenta volverá a estar activa y el usuario podrá acceder a la plataforma normalmente.';
                    })()}
                  </p>
                  <div className="flex gap-3">
                    <button onClick={() => setAccion(null)} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#0A0A0A', backgroundColor: '#F5F5F5', border: '1px solid #E5E5E5' }}>Cancelar</button>
                    <button onClick={handleSuspender} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#FFFFFF', backgroundColor: '#CA8A04' }}>Confirmar</button>
                  </div>
                </>
              ) : (
                <>
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ backgroundColor: '#E8F5EE' }}>
                    <User className="w-6 h-6" style={{ color: '#006B4E' }} />
                  </div>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '18px', fontWeight: 600, color: '#0A0A0A', margin: '0 0 4px' }}>Editar cuenta</h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#737373', margin: '0 0 20px' }}>{accion.nombre}</p>
                  <div className="space-y-3 mb-6">
                    {['Nombre', 'Email'].map(field => (
                      <div key={field}>
                        <label style={{ fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 600, color: '#0A0A0A', display: 'block', marginBottom: '4px' }}>{field}</label>
                        <input
                          type={field === 'Email' ? 'email' : 'text'}
                          defaultValue={field === 'Nombre' ? accion.nombre : ''}
                          placeholder={field === 'Email' ? 'correo@ejemplo.cl' : ''}
                          className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                          style={{ border: '1px solid #E5E5E5', fontFamily: 'var(--font-body)', color: '#0A0A0A' }}
                        />
                      </div>
                    ))}
                  </div>
                  <div className="flex gap-3">
                    <button onClick={() => setAccion(null)} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#0A0A0A', backgroundColor: '#F5F5F5', border: '1px solid #E5E5E5' }}>Cancelar</button>
                    <button onClick={() => { setAccion(null); showToast(`Cuenta de ${accion.nombre} actualizada`); }} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#FFFFFF', backgroundColor: '#006B4E' }}>Guardar cambios</button>
                  </div>
                </>
              )}
            </div>
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-3 rounded-xl z-50" style={{ backgroundColor: '#0A0A0A', color: '#FFFFFF', boxShadow: '0 8px 24px rgba(0,0,0,0.25)', whiteSpace: 'nowrap' }}>
          <CheckCircle className="w-4 h-4 flex-shrink-0" style={{ color: '#4ADE80' }} />
          <span style={{ fontFamily: 'var(--font-body)', fontSize: '13px' }}>{toast}</span>
        </div>
      )}
    </div>
  );
...
type PubEstado = 'activa' | 'suspendida' | 'pendiente';
type PubAccion = { tipo: 'eliminar' | 'suspender' | 'destacar'; id: number; titulo: string } | null;

const PUBLICACIONES_DATA = [
  { id: 1,  titulo: 'Parcela Vista Cordillera',  inmobiliaria: 'Vista Natura Propiedades', tipo: 'Parcela',  precio: '$45.000.000',  estado: 'activa' as PubEstado,    destacada: true,  fecha: '28 may 2025' },
  { id: 2,  titulo: 'Parcela Lago Azul',         inmobiliaria: 'Vista Natura Propiedades', tipo: 'Parcela',  precio: '$38.500.000',  estado: 'activa' as PubEstado,    destacada: false, fecha: '2 jun 2025' },
  { id: 3,  titulo: 'Proyecto Aysén Sur',         inmobiliaria: 'Sur Parcelas Ltda.',       tipo: 'Proyecto', precio: 'Desde $52M',   estado: 'activa' as PubEstado,    destacada: false, fecha: '10 jun 2025' },
  { id: 4,  titulo: 'Parcela Los Robles',         inmobiliaria: 'Inmobiliaria Verde Sur',   tipo: 'Parcela',  precio: '$29.000.000',  estado: 'activa' as PubEstado,    destacada: false, fecha: '14 jun 2025' },
  { id: 5,  titulo: 'Parcela El Valle Verde',     inmobiliaria: 'Vista Natura Propiedades', tipo: 'Parcela',  precio: '$33.000.000',  estado: 'suspendida' as PubEstado, destacada: false, fecha: '1 may 2025' },
  { id: 6,  titulo: 'Parcela Paine Norte',        inmobiliaria: 'Andina Terrenos',          tipo: 'Parcela',  precio: '$27.500.000',  estado: 'pendiente' as PubEstado, destacada: false, fecha: '12 jul 2025' },
  { id: 7,  titulo: 'Proyecto Villarrica Norte',  inmobiliaria: 'Sur Parcelas Ltda.',       tipo: 'Proyecto', precio: 'Desde $65M',   estado: 'activa' as PubEstado,    destacada: true,  fecha: '20 abr 2025' },
  { id: 8,  titulo: 'Parcela Sur Verde',          inmobiliaria: 'Inmobiliaria Verde Sur',   tipo: 'Parcela',  precio: '$18.000.000',  estado: 'suspendida' as PubEstado, destacada: false, fecha: '8 abr 2025' },
];

function CTPPublicacionesView() {
  const [publicaciones, setPublicaciones] = useState(PUBLICACIONES_DATA);
  const [filtroEstado, setFiltroEstado] = useState<'todas' | PubEstado>('todas');
  const [search, setSearch] = useState('');
  const [accion, setAccion] = useState<PubAccion>(null);
  const [toast, setToast] = useState<string | null>(null);
  ...
  function handleConfirmarAccion() {
    ...
    if (accion.tipo === 'eliminar') {
      setPublicaciones(prev => prev.filter(p => p.id !== accion.id));
      showToast(`Publicación eliminada`);
    } else if (accion.tipo === 'suspender') {
      setPublicaciones(prev => prev.map(p => {
        if (p.id !== accion.id) return p;
        return { ...p, estado: p.estado === 'activa' ? 'suspendida' : 'activa' };
      }));
      showToast(`Publicación ${publicaciones.find(p => p.id === accion.id)?.estado === 'activa' ? 'suspendida' : 'reactivada'}`);
    } else if (accion.tipo === 'destacar') {
      setPublicaciones(prev => prev.map(p => p.id === accion.id ? { ...p, destacada: !p.destacada } : p));
      showToast(`Publicación ${publicaciones.find(p => p.id === accion.id)?.destacada ? 'quitada de destacados' : 'marcada como destacada'}`);
    }
    setAccion(null);
  }
  ...
  const filtradas = publicaciones.filter(p => {
    const matchSearch = p.titulo.toLowerCase().includes(q) || p.inmobiliaria.toLowerCase().includes(q);
  ...
  const conteos = {
    ...
    activa:     publicaciones.filter(p => p.estado === 'activa').length,
    suspendida: publicaciones.filter(p => p.estado === 'suspendida').length,
    pendiente:  publicaciones.filter(p => p.estado === 'pendiente').length,
  };

  const estadoStyle = (estado: PubEstado) => ({
    activa:     { bg: '#DCFCE7', color: '#16A34A', label: 'Activa' },
    suspendida: { bg: '#FEE2E2', color: '#DC2626', label: 'Suspendida' },
    pendiente:  { bg: '#FEF3C7', color: '#B7791F', label: 'Pendiente' },
  }[estado]);

  const accionModal = accion ? publicaciones.find(p => p.id === accion.id) : null;

  return (
    <div className="p-4 md:p-8">
      <SectionShell title="Publicaciones" subtitle="Modera y gestiona todas las publicaciones de la plataforma." />
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {([
          ...
          <button
            ...
            style={{
              fontFamily: 'var(--font-body)',
              backgroundColor: filtroEstado === f.key ? '#006B4E' : '#F5F5F5',
              color: filtroEstado === f.key ? '#FFFFFF' : '#737373',
              border: filtroEstado === f.key ? '1px solid #006B4E' : '1px solid #E5E5E5',
            }}
          >
            {f.label}
            <span className="px-1.5 py-0.5 rounded-full text-xs" style={{ backgroundColor: filtroEstado === f.key ? 'rgba(255,255,255,0.2)' : '#E5E5E5', color: filtroEstado === f.key ? '#FFFFFF' : '#737373', fontWeight: 700 }}>
              {conteos[f.key]}
            </span>
          </button>
        ))}
      ...
      <div className="relative mb-4 max-w-xs">
        ...
        <input
          type="text"
          placeholder="Buscar publicación o inmobiliaria..."
          value={search}
          onChange={e => setSearch(e.target.value)}
          className="w-full pl-9 pr-3 py-2.5 rounded-xl text-sm outline-none"
          style={{ border: '1px solid #E5E5E5', fontFamily: 'var(--font-body)', color: '#0A0A0A', backgroundColor: '#FFFFFF' }}
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid #E5E5E5' }}>
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr style={{ backgroundColor: '#FAFAFA', borderBottom: '1px solid #E5E5E5' }}>
                {['Publicación', 'Inmobiliaria', 'Tipo', 'Precio', 'Estado', 'Fecha', 'Acciones'].map(h => (
                  <th key={h} className="px-5 py-3 text-left" style={{ fontSize: '11px', fontWeight: 600, color: '#737373', fontFamily: 'var(--font-body)', letterSpacing: '0.04em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <FolderOpen className="w-8 h-8" style={{ color: '#D4D4D4' }} />
                      <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#A3A3A3', margin: 0 }}>Sin publicaciones en esta categoría</p>
                    </div>
                  </td>
                </tr>
              ) : filtradas.map((p, idx) => {
                const s = estadoStyle(p.estado);
                return (
                  <tr key={p.id} style={{ borderBottom: idx < filtradas.length - 1 ? '1px solid #F3F4F6' : 'none' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#FAFAFA'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#FFFFFF'; }}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <span style={{ fontSize: '13px', fontWeight: 600, color: '#0A0A0A', fontFamily: 'var(--font-body)' }}>{p.titulo}</span>
                        {p.destacada && (
                          <span className="px-1.5 py-0.5 rounded text-xs font-semibold" style={{ backgroundColor: '#FEF9C3', color: '#CA8A04', fontFamily: 'var(--font-body)' }}>Destacada</span>
                        )}
                      </div>
                    </td>
                    <td className="px-5 py-4"><span style={{ fontSize: '13px', color: '#737373', fontFamily: 'var(--font-body)' }}>{p.inmobiliaria}</span></td>
                    <td className="px-5 py-4"><span style={{ fontSize: '12px', fontWeight: 600, padding: '2px 8px', borderRadius: '99px', fontFamily: 'var(--font-body)', backgroundColor: p.tipo === 'Proyecto' ? '#EFF6FF' : '#F5F5F5', color: p.tipo === 'Proyecto' ? '#2563EB' : '#737373' }}>{p.tipo}</span></td>
                    <td className="px-5 py-4"><span style={{ fontSize: '13px', color: '#0A0A0A', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>{p.precio}</span></td>
                    <td className="px-5 py-4"><span className="px-2.5 py-0.5 rounded-full text-xs font-semibold" style={{ fontFamily: 'var(--font-body)', backgroundColor: s.bg, color: s.color }}>{s.label}</span></td>
        ...
        <div className="md:hidden">
          {filtradas.length === 0 ? (
            <div className="p-8 text-center">
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#A3A3A3' }}>Sin publicaciones en esta categoría</p>
            </div>
          ) : filtradas.map((p, idx) => {
            const s = estadoStyle(p.estado);
            return (
              <div key={p.id} className="p-4" style={{ borderBottom: idx < filtradas.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                <div className="flex items-start justify-between mb-1">
                  <p style={{ fontWeight: 600, fontSize: '14px', color: '#0A0A0A', fontFamily: 'var(--font-body)', margin: 0, flex: 1, paddingRight: '8px' }}>{p.titulo}</p>
            ...
            <div className="p-6">
              {accion.tipo === 'eliminar' ? (
                <>
                  ...
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '18px', fontWeight: 600, color: '#0A0A0A', margin: '0 0 8px' }}>¿Eliminar publicación?</h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#737373', lineHeight: '1.6', margin: '0 0 20px' }}>
                    <strong style={{ color: '#0A0A0A' }}>{accion.titulo}</strong> será eliminada permanentemente. Esta acción no se puede deshacer.
                  </p>
                  <div className="flex gap-3">
                    <button onClick={() => setAccion(null)} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#0A0A0A', backgroundColor: '#F5F5F5', border: '1px solid #E5E5E5' }}>Cancelar</button>
                    <button onClick={handleConfirmarAccion} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#FFFFFF', backgroundColor: '#DC2626' }}>Eliminar</button>
                  </div>
                </>
              ) : accion.tipo === 'suspender' ? (
                <>
                  ...
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '18px', fontWeight: 600, color: '#0A0A0A', margin: '0 0 8px' }}>
                    {accionModal?.estado === 'activa' ? '¿Suspender publicación?' : '¿Reactivar publicación?'}
                  </h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#737373', lineHeight: '1.6', margin: '0 0 20px' }}>
                    {accionModal?.estado === 'activa'
                      ? `"${accion.titulo}" dejará de ser visible para los usuarios hasta que se reactive.`
                      : `"${accion.titulo}" volverá a estar visible para todos los usuarios.`}
                  </p>
                  <div className="flex gap-3">
                    <button onClick={() => setAccion(null)} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#0A0A0A', backgroundColor: '#F5F5F5', border: '1px solid #E5E5E5' }}>Cancelar</button>
                    <button onClick={handleConfirmarAccion} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#FFFFFF', backgroundColor: '#CA8A04' }}>Confirmar</button>
                  </div>
                </>
              ) : (
                <>
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ backgroundColor: '#FEF9C3' }}>
                    <Bookmark className="w-6 h-6" style={{ color: '#CA8A04' }} />
                  </div>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '18px', fontWeight: 600, color: '#0A0A0A', margin: '0 0 8px' }}>
                    {accionModal?.destacada ? 'Quitar de destacados' : 'Destacar publicación'}
                  </h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#737373', lineHeight: '1.6', margin: '0 0 20px' }}>
                    {accionModal?.destacada
                      ? `"${accion.titulo}" dejará de aparecer en posiciones destacadas de la plataforma.`
                      : `"${accion.titulo}" aparecerá en posiciones destacadas para los usuarios.`}
                  </p>
                  <div className="flex gap-3">
                    <button onClick={() => setAccion(null)} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#0A0A0A', backgroundColor: '#F5F5F5', border: '1px solid #E5E5E5' }}>Cancelar</button>
                    <button onClick={handleConfirmarAccion} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ fontFamily: 'var(--font-body)', color: '#FFFFFF', backgroundColor: '#006B4E' }}>Confirmar</button>
                  </div>
                </>
              )}
            </div>
