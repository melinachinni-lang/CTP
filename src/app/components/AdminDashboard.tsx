...
import { Home, BarChart3, TrendingDown, FolderOpen, DollarSign, ChevronDown, Calendar, Monitor, Smartphone, TrendingUp, ArrowUpRight, ArrowDownRight, AlertTriangle, Users, UserPlus, UserCheck, Activity, MousePointer, FileText, Send, MessageCircle, Eye, Mail, Phone, Globe, Search, Download, Filter, Image as ImageIcon, Save, Edit, X, Layout, Video, MoveUp, MoveDown, ToggleLeft, ToggleRight, Edit2, Settings, Shield, Zap, Link, ChevronRight, Plus, MapPin, Maximize2, Trash2, MoreVertical, Building2, BookOpen, Upload, Tag, EyeOff, Check, AlertCircle } from 'lucide-react';
...
type NavItem = 'inicio' | 'leads' | 'analitica' | 'embudo' | 'proyectos' | 'publicaciones' | 'recursos' | 'configuracion';
                  ...
                  <div className="divide-y" style={{ borderColor: '#F5F5F5' }}>
                    ...
                    {recursosFiltrados.map((recurso) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <button title="Editar" onClick={() => openEditarRecurso(recurso)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ backgroundColor: '#F5F5F5', color: '#737373' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#E5E5E5'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#F5F5F5'; }}>
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button title={recurso.estado === 'activo' ? 'Desactivar' : 'Activar'} onClick={() => toggleEstadoRecurso(recurso.id)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ backgroundColor: recurso.estado === 'activo' ? '#F0F5EB' : '#F5F5F5', color: recurso.estado === 'activo' ? '#3D5E28' : '#A3A3A3' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = recurso.estado === 'activo' ? '#E2EDCC' : '#E5E5E5'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = recurso.estado === 'activo' ? '#F0F5EB' : '#F5F5F5'; }}>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                  <div className="w-full max-w-2xl rounded-2xl overflow-hidden" style={{ backgroundColor: '#FFFFFF', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
                    <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid #E5E5E5' }}>
                      ...
                      <button onClick={() => setShowRecursoModal(false)} className="w-8 h-8 rounded-full flex items-center justify-center transition-colors" style={{ backgroundColor: '#F5F5F5', color: '#737373' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#E5E5E5'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#F5F5F5'; }}>
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="px-6 py-6 space-y-5 overflow-y-auto" style={{ maxHeight: '70vh' }}>
                      <div>
                        ...
                        <input type="text" placeholder="Ej: Cómo invertir en parcelas en Chile" value={recursoForm.titulo} onChange={e => setRecursoForm(f => ({ ...f, titulo: e.target.value }))} className="w-full px-4 py-3 rounded-xl outline-none transition-all" style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#0A0A0A', backgroundColor: '#FAFAFA', border: `1px solid ${recursoFormErrors.titulo ? '#EF4444' : '#E5E5E5'}` }} onFocus={e => { e.target.style.borderColor = '#3D5E28'; e.target.style.backgroundColor = '#FFFFFF'; }} onBlur={e => { e.target.style.borderColor = recursoFormErrors.titulo ? '#EF4444' : '#E5E5E5'; e.target.style.backgroundColor = '#FAFAFA'; }} />
                        {recursoFormErrors.titulo && <p className="mt-1.5 flex items-center gap-1" style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-xs)', color: '#EF4444' }}><AlertCircle className="w-3 h-3" /> El título es obligatorio</p>}
                      </div>
                      <div>
                        <label style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', fontWeight: 'var(--font-weight-medium)', color: '#0A0A0A', display: 'block', marginBottom: '6px' }}>
                          Descripción / Contenido <span style={{ color: '#EF4444' }}>*</span>
                        </label>
                        <textarea placeholder="Escribí el contenido o descripción del recurso..." value={recursoForm.descripcion} onChange={e => setRecursoForm(f => ({ ...f, descripcion: e.target.value }))} rows={5} className="w-full px-4 py-3 rounded-xl outline-none transition-all resize-none" style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#0A0A0A', backgroundColor: '#FAFAFA', border: `1px solid ${recursoFormErrors.descripcion ? '#EF4444' : '#E5E5E5'}`, lineHeight: 'var(--line-height-body)' }} onFocus={e => { e.target.style.borderColor = '#3D5E28'; e.target.style.backgroundColor = '#FFFFFF'; }} onBlur={e => { e.target.style.borderColor = recursoFormErrors.descripcion ? '#EF4444' : '#E5E5E5'; e.target.style.backgroundColor = '#FAFAFA'; }} />
                        {recursoFormErrors.descripcion && <p className="mt-1.5 flex items-center gap-1" style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-xs)', color: '#EF4444' }}><AlertCircle className="w-3 h-3" /> La descripción es obligatoria</p>}
                      </div>
                      <div>
                        <label style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', fontWeight: 'var(--font-weight-medium)', color: '#0A0A0A', display: 'block', marginBottom: '6px' }}>Tópico</label>
                        <select value={recursoForm.topico} onChange={e => setRecursoForm(f => ({ ...f, topico: e.target.value }))} className="w-full px-4 py-3 rounded-xl outline-none cursor-pointer" style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#0A0A0A', backgroundColor: '#FAFAFA', border: '1px solid #E5E5E5' }}>
                          {topicosRecurso.map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                      </div>
                      <div>
                        <label style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', fontWeight: 'var(--font-weight-medium)', color: '#0A0A0A', display: 'block', marginBottom: '6px' }}>Imagen destacada</label>
                        <div className="w-full rounded-xl flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors" style={{ border: '2px dashed #C5D9A8', backgroundColor: '#F0F5EB', padding: '32px 16px', minHeight: '130px' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#E2EDCC'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#F0F5EB'; }}>
                          ...
                          <div className="text-center">
                            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', fontWeight: 'var(--font-weight-medium)', color: '#3D5E28' }}>Subir imagen</p>
                            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-xs)', color: '#737373', marginTop: '2px' }}>PNG, JPG o WEBP · Máx 5 MB · Recomendado 1200×630px</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center justify-between px-6 py-4" style={{ borderTop: '1px solid #E5E5E5' }}>
                      <button onClick={() => setShowRecursoModal(false)} className="px-4 py-2.5 rounded-xl text-sm transition-colors" style={{ fontFamily: 'var(--font-body)', color: '#737373', backgroundColor: '#F5F5F5', border: '1px solid #E5E5E5' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#E5E5E5'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#F5F5F5'; }}>Cancelar</button>
                      <button onClick={guardarRecurso} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all" style={{ backgroundColor: recursoSaveSuccess ? '#166534' : '#3D5E28', color: '#FFFFFF', fontFamily: 'var(--font-body)' }} onMouseEnter={e => { if (!recursoSaveSuccess) e.currentTarget.style.backgroundColor = '#2E4A1E'; }} onMouseLeave={e => { if (!recursoSaveSuccess) e.currentTarget.style.backgroundColor = '#3D5E28'; }}>
                        {recursoSaveSuccess ? <><Check className="w-4 h-4" /> Guardado</> : <><Save className="w-4 h-4" /> {recursoEditing ? 'Guardar cambios' : 'Publicar recurso'}</>}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                  <div className="w-full max-w-sm rounded-2xl overflow-hidden" style={{ backgroundColor: '#FFFFFF', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
                    <div className="p-6">
                      ...
                      <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 'var(--font-size-h4)', fontWeight: 'var(--font-weight-medium)', color: '#0A0A0A', marginBottom: '8px' }}>¿Eliminar este recurso?</h3>
                      ...
                      <div className="flex gap-3">
                        <button onClick={() => setDeleteConfirmId(null)} className="flex-1 py-2.5 rounded-xl text-sm transition-colors" style={{ fontFamily: 'var(--font-body)', fontWeight: 'var(--font-weight-medium)', color: '#0A0A0A', backgroundColor: '#F5F5F5', border: '1px solid #E5E5E5' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#E5E5E5'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#F5F5F5'; }}>Cancelar</button>
                        <button onClick={() => eliminarRecurso(deleteConfirmId)} className="flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors" style={{ fontFamily: 'var(--font-body)', color: '#FFFFFF', backgroundColor: '#EF4444' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#DC2626'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#EF4444'; }}>Sí, eliminar</button>
                      </div>
                    </div>
                  </div>
                </div>
