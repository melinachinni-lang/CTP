...
export function NewListingFlow({ onClose, onPublish, parcelaId, userType }: NewListingFlowProps) {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      if (isBroker && !brokerInmobiliaria) {
        newErrors.brokerInmobiliaria = 'Inmobiliaria: selecciona la inmobiliaria a la que pertenece esta parcela';
      }
        ...
        <div className="max-w-[1000px] mx-auto px-6 py-8 pb-32">
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              ...
              {/* Inmobiliaria (solo broker) */}
              {isBroker && (
                <div className="p-6 rounded-xl space-y-4" style={{ backgroundColor: '#FFFFFF', border: '1px solid var(--border)' }}>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 'var(--font-size-h4)', fontWeight: 'var(--font-weight-medium)', color: '#0A0A0A' }}>
                    Inmobiliaria asociada
                  </h3>
                  <div>
                    <label className="block mb-1.5" style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', fontWeight: 500, color: '#374151' }}>
                    ...
                    <div className="relative">
                      <button
                        ...
                        style={{
                          border: `1.5px solid ${errors.brokerInmobiliaria ? '#DC2626' : showBrokerDropdown ? '#006B4E' : brokerInmobiliaria ? '#006B4E' : '#DEDEDE'}`,
              ...
              <div className="p-6 rounded-xl space-y-4" style={{ backgroundColor: '#FFFFFF', border: '1px solid var(--border)' }}>
                ...
                >
                  <div className="w-5 h-5 rounded flex items-center justify-center flex-shrink-0" style={{ backgroundColor: formData.hasFinancing ? '#647E3F' : '#FFFFFF', border: `2px solid ${formData.hasFinancing ? '#647E3F' : '#DEDEDE'}` }}>
                    {formData.hasFinancing && <Check className="w-3 h-3" style={{ color: '#FFFFFF' }} />}
                  </div>
                  <div>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', fontWeight: 500, color: '#0A0A0A' }}>
                      Esta parcela tiene financiamiento disponible
                    </p>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: '#737373', marginTop: '2px' }}>
                      Configura los planes de cuotas, pie mínimo y tasa de interés
                    </p>
                  </div>
                </div>
                ...
                {formData.hasFinancing && (
                  <div className="space-y-3">
                        ...
                        {formData.financingPlans.map((plan, idx) => (
                          <div key={idx} className="flex items-center justify-between px-4 py-3 rounded-lg" style={{ backgroundColor: '#F0F7F0', border: '1px solid #C6DFC8' }}>
                            <div className="flex items-center gap-4 flex-wrap">
                              <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#0A0A0A', fontWeight: 500 }}>Plan {idx + 1}</span>
                              <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#374151' }}>Pie mín. ${parseInt(plan.pieMinimoCLP || '0').toLocaleString('es-CL')}</span>
                              <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#374151' }}>{plan.cuotas} cuotas</span>
                              <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#374151' }}>Tasa {plan.tasa}% anual</span>
                            </div>
                            <button onClick={() => setFormData(prev => ({ ...prev, financingPlans: prev.financingPlans.filter((_, i) => i !== idx) }))}
                              className="p-1.5 rounded-md transition-colors" style={{ color: '#6B7280' }}
                    ...
                    {showAddPlan && (
                      <div className="p-4 rounded-lg space-y-3" style={{ backgroundColor: '#FAFAFA', border: '1px solid #DEDEDE' }}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          <div>
                            <label style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: '12px', color: '#6B7280', marginBottom: '4px' }}>Pie mínimo (CLP)</label>
                            <input type="text" value={newPlan.pieMinimoCLP} onChange={e => setNewPlan({ ...newPlan, pieMinimoCLP: e.target.value })} placeholder="Ej: 5000000"
                              className="w-full px-3 py-2 rounded-lg" style={{ backgroundColor: '#FFFFFF', border: '1px solid #DEDEDE', fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#0A0A0A', outline: 'none' }} />
                          </div>
                          <div>
                            <label style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: '12px', color: '#6B7280', marginBottom: '4px' }}>Cantidad de cuotas</label>
                            <input type="number" value={newPlan.cuotas} onChange={e => setNewPlan({ ...newPlan, cuotas: e.target.value })} placeholder="Ej: 120"
                              className="w-full px-3 py-2 rounded-lg" style={{ backgroundColor: '#FFFFFF', border: '1px solid #DEDEDE', fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#0A0A0A', outline: 'none' }} />
                          </div>
                          <div>
                            <label style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: '12px', color: '#6B7280', marginBottom: '4px' }}>Tasa anual (%)</label>
                            <input type="number" step="0.1" value={newPlan.tasa} onChange={e => setNewPlan({ ...newPlan, tasa: e.target.value })} placeholder="Ej: 4.5"
                              className="w-full px-3 py-2 rounded-lg" style={{ backgroundColor: '#FFFFFF', border: '1px solid #DEDEDE', fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-body-sm)', color: '#0A0A0A', outline: 'none' }} />
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => { if (newPlan.pieMinimoCLP && newPlan.cuotas && newPlan.tasa) { setFormData(prev => ({ ...prev, financingPlans: [...prev.financingPlans, { ...newPlan }] })); setNewPlan({ pieMinimoCLP: '', cuotas: '', tasa: '' }); setShowAddPlan(false); } }}
                            className="px-4 py-2 rounded-lg text-sm font-medium" style={{ backgroundColor: '#647E3F', color: '#FFFFFF', fontFamily: 'var(--font-body)' }}>
                            Guardar plan
                          </button>
                          {formData.financingPlans.length > 0 && (
                            <button onClick={() => { setShowAddPlan(false); setNewPlan({ pieMinimoCLP: '', cuotas: '', tasa: '' }); }}
                              className="px-4 py-2 rounded-lg text-sm" style={{ backgroundColor: '#F5F5F5', color: '#374151', fontFamily: 'var(--font-body)' }}>
                              Cancelar
                            </button>
                          )}
                        </div>
                    ...
                    {!showAddPlan && formData.financingPlans.length > 0 && (
                      <button onClick={() => setShowAddPlan(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm" style={{ color: '#647E3F', fontFamily: 'var(--font-body)', fontWeight: 500 }}>
